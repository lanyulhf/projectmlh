...
define(function(require,exports,module){
	...
	let util = require("utilcmd");
	class Slider{
		constructor(selectorStr,width,height,imgs,hrefs,timeSpace){
			this.boxDom = util.$(selectorStr);
			this.width = width;
			this.height = height;
			this.imgs = imgs;
			this.hrefs = hrefs;
		...
		initUI(){
			...
			for(let i=0;i<this.imgs.length;i++){
				let imgDom = util.$create("img");
				imgDom.src = this.imgs[i];
				imgDom.style.cssText = "position:absolute;left:"+this.width+"px;top:0px;width:100%;height:524px;";
			...
			//2)、创建豆豆：
			let ulDom = util.$create("ul");
			ulDom.style.cssText = "position:absolute;right:670px;bottom:10px;height:40px;list-style:none;z-index:999;";
			this.boxDom.appendChild(ulDom);
			for(let i=0;i<this.imgs.length;i++){
				let liDom = util.$create("li");
				liDom.style.cssText = "float:left;margin-left:10px;width:20px;height:20px;border-radius:50%;background-color:orange;";
				ulDom.appendChild(liDom);
			}		
			ulDom.children[0].style.backgroundColor = "red";
			/*let arrowLeftDom = util.$create("div");
			arrowLeftDom.style.cssText = "position:absolute;right:500px;bottom:200px;height:40px;width:25px;z-index:999;";
			this.boxDom.appendChild(arrowLeftDom);
			imgDom.src = "../img/bigbanner_leftarrows.png";*/
